  ...
  if (!lesson) {
    return (
      <div className="empty-state">
        ...
        <button className="btn btn-secondary" onClick={() => navigate('/learning/tree')}>Back to Learning Path</button>
      </div>
    )
  }
    ...
    <div>
      <div style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <button className="btn btn-secondary" onClick={() => navigate('/learning/tree')}>← Back</button>
          <span style={{ fontSize: 13, color: '#888' }}>{lesson.duration} · Level {lesson.level}</span>
        </div>
      ...
      {!quizActive ? (
          ...
          <div className="card" style={{ marginBottom: 20 }}>
            ...
            {section.examples && (
              <div style={{ marginTop: 16 }}>
                <div style={{ fontSize: 13, color: '#888', marginBottom: 8 }}>Examples:</div>
        ...
        <div className="card">
          <h3 style={{ marginBottom: 16 }}>
            {quizDone ? 'Quiz Results' : `Quiz — ${lesson.quiz.length} questions`}
          </h3>
          ...
          {quizDone && (
            <div style={{ marginTop: 20, padding: 20, background: 'rgba(0,0,0,0.2)', borderRadius: 8, textAlign: 'center' }}>
              <div style={{ fontSize: 14, color: '#888', marginBottom: 4 }}>Score</div>
              <div style={{ fontSize: 36, fontWeight: 700, color: 'var(--sanskrit-gold)' }}>
                {Math.round(quizScore * 100)}%
              </div>
              <div style={{ fontSize: 13, color: '#888', marginTop: 8 }}>
                +{Math.round(quizScore * 10 * (1 + (lesson.quiz[0]?.difficulty ?? 1) * 0.5))} XP earned
              </div>
